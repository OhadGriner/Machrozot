...
async def test_create_puzzle_returns_derived_words(api: AsyncClient, puzzle_payload, admin_headers):
    response = await api.post("/api/puzzle", json=puzzle_payload, headers=admin_headers)
...
async def test_puzzle_response_includes_cell_paths_for_position_validation(api: AsyncClient, puzzle_payload, admin_headers):
...
async def test_get_puzzle_by_id(api: AsyncClient, puzzle_payload, admin_headers):
    created = (await api.post("/api/puzzle", json=puzzle_payload, headers=admin_headers)).json()
...
async def test_get_today_puzzle(api: AsyncClient, puzzle_payload, admin_headers):
    puzzle_payload["scheduled_date"] = str(date.today())
    await api.post("/api/puzzle", json=puzzle_payload, headers=admin_headers)
...
async def test_get_puzzle_by_date(api: AsyncClient, puzzle_payload, admin_headers):
...
async def test_get_puzzle_by_date_rejects_future_dates_even_when_scheduled(
    api: AsyncClient, puzzle_payload, admin_headers
):
...
async def test_get_puzzle_by_date_allows_today(api: AsyncClient, puzzle_payload, admin_headers):
...
async def test_create_puzzle_requires_admin(api: AsyncClient, puzzle_payload):
    # This endpoint used to be completely unauthenticated — regression-guard
    # that anonymous puzzle creation stays impossible.
