...
**Machrozot** (מחרוזות) — a full-stack Hebrew word-search game. Players find themed words in an 8×6 letter grid where every cell is used exactly once. A special "mega machrozet" (מגה מחרוזת) word spans the board and reveals the theme.
...
- `GET /api/puzzle/today` — includes `mega_machrozet_cells`/`word_cells` (the ordered answer paths). The board JSON is fully inspectable by design — answer *strings* (`mega_machrozet`, `words`) were already sent before this; withholding only positions gave a false sense of security while still letting a repeated letter be selected from the wrong cell and accepted.
- Word validation is client-side, by **position and order**: compare the selected `{row, col}` path against `puzzle.mega_machrozet_cells` / `puzzle.word_cells` exactly (not reversed) — not by concatenating letters and comparing strings. String-only comparison is exploitable whenever a letter repeats on the board.
...
**Word check** (`useWordCheck.ts`): normalize → compare. Mega machrozet match → gold cell state. Non-theme valid words ≥4 letters increment `nonThemeCount`; every 3 earns 1 hint.
